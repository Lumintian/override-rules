...
`buildCountryGroups()` 按地区权重遍历至少有一个候选节点的地区：基础组遵循 `countryThreshold` 和 `groupType`，额外组按显式数量生成，固定为 `select`，名称为「地区额外1」「地区额外2」等。无该地区节点时不生成空组。只要某地区生成了基础组或至少一个额外组，该地区节点就不再重复加入 `手动选择`；未生成任何地区组和未识别地区的节点仍保留，没有剩余候选节点时不生成空的 `手动选择` 组。两类组均遵循 `regexFilter`，所以正则模式也保留运行时独立匹配及不按 `dialer-proxy` 排除成员的现有行为。
