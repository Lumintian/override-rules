...
我们使用 TypeScript 进行源码管理。Mihomo 覆写逻辑位于 `src/`，Sub-Store 节点重命名逻辑位于 `scripts/substore/rename.ts`。**请务必不要直接修改根目录下的 `.js` 和 `.yaml` 产物文件**。
...
脚本会根据 `countriesMeta` 自动生成实际存在的代理组。只要某地区生成了基础地区组或至少一个额外组，该地区节点就不会再重复出现在 `手动选择`；未生成任何地区组或未识别地区的节点仍会保留，没有剩余候选节点时不会生成空的 `手动选择` 组。调整匹配规则时修改元数据即可；新增地区时，还需在 `src/types.ts` 的 `CountryCode` 中加入对应参数代码，并同步 [`CONFIGURATION.md`](./CONFIGURATION.md) 的参数表。
...
开发过程中可以运行 `npm run preview`，在本地网页中输入节点名称并即时查看当前改名 / 覆写源码的结果，保存业务源码后会自动重新预览。预览器不会写入构建产物，详情见 [本地开发预览](PREVIEW.md)。
