...
脚本会根据 `countriesMeta` 自动生成实际存在的代理组。只要某地区生成了基础地区组或至少一个额外组，该地区节点就不会再重复出现在 `手动选择`；未生成任何地区组或未识别地区的节点仍会保留，没有剩余候选节点时不会生成空的 `手动选择` 组。调整匹配规则时修改元数据即可；新增地区时，还需在 `src/types.ts` 的 `CountryCode` 中加入对应参数代码，并同步 README 的参数表。
