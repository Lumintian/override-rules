...
第一次同时配置 provider 与前置代理，请直接照做 [README 完整示例](../README.md#providerurl-与前置代理完整示例)。本节用来解释组的用途，不需要把这里的参数再重复执行一遍。

```text
你的设备 → 前置节点（负责连接落地）→ 落地节点（最终出口）→ 目标网站
```

| 策略组 | 选什么 |
| --- | --- |
| `前置代理A` | 一个能连接落地节点的普通节点，例如香港或新加坡节点 |
| `落地节点A` | 最终出口，例如美国自建节点 |
| `选择代理` 或某个服务分流组 | 选择 `落地节点A`，让该流量使用整条链路；只选前置组不会再经过落地 |

**`中转A` 标签加在落地节点上，不是前置节点上。** 落地节点可以直接携带 `dialer-proxy: "前置代理"`，也可以由改名脚本的 `chain` 参数根据原始名称或 `blkey` 输出标签写入。带字母标签的节点会形成独立链路：
...
使用 provider 时，来源既要有 `dialer-proxy`，也要在最终名称中保留对应中转标签；rename 因此需要同时设置 `chain` 和保留标签的 `blkey`。前置与普通地区候选会排除所有中转节点。两端都在 provider、或一端为显式节点都可使用；只有 `providerurl` 对应的已校验来源进入动态链路组。

注意：没有前置候选时仍会回退**直连**，不是阻断连接；新增链路编号需要重新生成主配置。遇到空组、缺组或出口不符，先看[快速排错](./CONFIGURATION.md#快速排错)；其他限制见 [provider 链式代理](./CONFIGURATION.md#provider-链式代理)。
