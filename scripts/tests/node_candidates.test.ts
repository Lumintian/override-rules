...
import { describeProxyNode, getActiveCountryNames, parseCountries, parseNodesByLanding, parseTailscale, sortProxyNodes } from "../../src/node_parser";
...
test("group and direct-node ordering share HK/SG preferences", () => {
    ...
    assert.deepEqual(names(sortProxyNodes(input)), ["香港 01", "新加坡 01", "台湾 01", "日本 01", "美国 01"]);
    assert.deepEqual(getActiveCountryNames(parseCountries(input), 1), ["香港", "新加坡", "台湾", "日本", "美国"]);
    for (const [country, meta] of Object.entries(countriesMeta)) assert.equal(meta.weight, countryWeights[country]);
});

test("merged candidates cluster regions, provider prefixes and categories without renaming", () => {
    const input = nodes(["A | 香港 0.2× 01", "A | 美国 01", "B | 香港 01", "A | 香港 01", "B | 新加坡 01"]);
    const before = structuredClone(input);
    const result = sortProxyNodes(input);
    assert.deepEqual(names(result), ["A | 香港 01", "A | 香港 0.2× 01", "B | 香港 01", "B | 新加坡 01", "A | 美国 01"]);
...
test("existing city aliases and lowercase airport codes remain recognized", () => {
    for (const [name, country] of [["A | 洛杉矶 01", "美国"], ["A | 马赛 01", "法国"], ["A | muc 01", "德国"], ["A | 关西 01", "日本"]]) {
...
test("long known names must not fall into short substring country matchers", () => {
    assert.equal(parseCountries(nodes(["亚美尼亚", "白俄罗斯", "法罗群岛", "英属维京群岛"])).美国, undefined);
    assert.deepEqual(Object.keys(parseCountries(nodes(["亚美尼亚", "白俄罗斯", "法罗群岛", "英属维京群岛"]))), []);
...
test("unregistered countries sort before unknowns without creating unsupported groups", () => {
    ...
    assert.deepEqual(names(sortProxyNodes(input)), ["日本 01", "越南 01", "越南 02", "unknown-z", "unknown-a"]);
...
test("sorting preserves landing and Tailscale membership and connection fields", () => {
    const input = [{ name: "美国 01", "dialer-proxy": "前置代理", server: "test.invalid" }, { name: "香港 01", type: "ss" }, { name: "私网", type: "tailscale" }];
    const result = sortProxyNodes(input);
