...
(() => {
  ...
  // shared/chain_tags.ts
  var FRONT_PROXY_PREFIX = "前置代理";
  var TRANSIT_PREFIX = "中转";
  function readTransitTags(name) {
    return Array.from(
  ...
  var TRANSIT_PATTERN = "中转[A-Za-z]?(?:$|[^A-Za-z0-9])";
  function transitPattern(id) {
    if (!/^[A-Z]?$/.test(id)) throw new Error("Invalid chain identifier");
    return `${TRANSIT_PREFIX}${id}(?:$|[^A-Za-z0-9])`;
  }
  var CHAIN_MATCHERS = ["", ..."ABCDEFGHIJKLMNOPQRSTUVWXYZ"].map((id) => ({
    id,
    regex: new RegExp(transitPattern(id), "i")
  }));
  function hasConsistentTransitIdentity(name, dialer) {
    const tags = [...new Set(readTransitTags(name))];
    const ids = CHAIN_MATCHERS.filter(({ regex }) => regex.test(name)).map(({ id: id2 }) => id2);
    if (dialer === void 0 || dialer === "") return tags.length === 0 && ids.length === 0;
    const id = landingChainId(dialer);
    return id !== null && dialer === `${FRONT_PROXY_PREFIX}${id}` && tags.length === 1 && tags[0] === `${TRANSIT_PREFIX}${id}` && ids.length === 1 && ids[0] === id;
  }
  ...
  // src/proxy_groups.ts
  function excludeProviderLandings(patterns = []) {
    return `(?i)${[TRANSIT_PATTERN, ...patterns].filter(Boolean).map((pattern) => `(?:${pattern})`).join("|")}`;
  }
  function providerFrontSource(providerNames, countries) {
    if (providerNames.length === 0) return {};
    const meta = countries?.map((country) => countriesMeta[country]).filter(Boolean);
    return {
      use: providerNames,
      ...meta && {
        filter: meta.length > 0 ? `(?i)${meta.map(({ pattern }) => `(?:${pattern})`).join("|")}` : "^$"
      },
  ...
  function buildCountryGroups({
    ...
    return getActiveCountryNames(countryNodes, 1).flatMap((country) => {
      ...
      const nodeSource = providerNames.length > 0 ? {
      ...
      ...landingChains.flatMap((chain) => [
        {
          ...
          proxies: chainProviderNames.length > 0 ? (frontProxySelectors[chain.id] ?? []).filter((name) => name !== "DIRECT") : frontProxySelectors[chain.id] ?? ["DIRECT"],
  ...
  function getProxyProviders(config) {
    ...
    for (const [name, provider] of Object.entries(providers)) {
      if (!name.trim() || !provider || typeof provider !== "object" || !["http", "file", "inline"].includes(provider.type) || provider.type === "http" && (typeof provider.url !== "string" || !provider.url) || provider.type === "file" && (typeof provider.path !== "string" || !provider.path) || provider.type === "inline" && !Array.isArray(provider.payload)) {
  ...
  function prepareProvider(config, args) {
    if (config.proxies !== void 0 && !Array.isArray(config.proxies)) {
    ...
    if (!["http:", "https:"].includes(url.protocol) || url.username || url.password || url.hash || /[\s\u0000-\u001f\u007f]/.test(args.providerurl)) {
      fail("providerurl 仅支持不含用户名、密码、Fragment 或空白的 HTTP(S) URL。");
    }
    const interval = args.providerinterval === void 0 ? 3600 : Number(args.providerinterval);
    ...
    if (existing) {
      if (["filter", "exclude-filter", "exclude-type", "override"].some(
    ...
    const paths = Object.values(providers).map((provider) => provider.path).filter((path2) => typeof path2 === "string").map(normalizePath);
    const pathTaken = (path2) => paths.some(
      (used) => used === normalizePath(path2) || used.endsWith(`/${normalizePath(path2)}`)
    );
  ...
  function snapshotHeaders(config, url) {
    const provider = Object.values(getProxyProviders(config)).find(
      (item) => item.type === "http" && item.url === url
    );
    const headers = /* @__PURE__ */ Object.create(null);
    if (provider.header !== void 0) {
      if (!provider.header || typeof provider.header !== "object" || Array.isArray(provider.header)) {
  ...
  async function downloadSnapshot(url, runtime, headers = { "User-Agent": "clash.meta" }) {
    let response;
    try {
    ...
    if (response.body.length > SNAPSHOT_MAX_BYTES || runtime.byteLength(response.body) > SNAPSHOT_MAX_BYTES) {
    ...
    for (const node of nodes) {
      ...
      if (!node || typeof node !== "object" || typeof node.name !== "string" || !node.name.trim() || node.name.length > 512 || typeof node.type !== "string" || !node.type.trim() || typeof node.server !== "string" || !node.server.trim() || !Number.isInteger(node.port) || node.port < 1 || node.port > 65535 || names.has(node.name)) {
  ...
  function validateSnapshotChains(nodes) {
  ...
  function subStoreRuntime() {
    if (typeof $substore === "undefined" || !$substore.env?.isNode || typeof $substore.http?.get !== "function" || typeof ProxyUtils === "undefined" || typeof ProxyUtils.yaml?.safeLoad !== "function" || typeof Buffer === "undefined") {
      fail(
  ...
  function main(config, args = getRawArgs(), snapshotNodes = []) {
    const {
      groupType,
      ipv6Enabled,
      fullConfig,
    ...
    if (config.proxies !== void 0 && !Array.isArray(config.proxies) || config.proxies === void 0 && providerNames.length === 0) {
      throw new Error(
    ...
    const snapshotProvider = snapshotNodes.length > 0 ? Object.entries(providers).find(
      ([, provider]) => provider.type === "http" && provider.url === args.providerurl
    ) : void 0;
    ...
    const landingChains = providerNames.length > 0 ? parseNodesByLanding([...proxies, ...snapshotNodes]).landingChains.map((chain) => ({
      ...chain,
    ...
    return {
      proxies,
      ...config["proxy-providers"] !== void 0 && { "proxy-providers": providers },
  ...
  function runMain(config, args = getRawArgs()) {
    if (args.providerurl === void 0) {
      if (args.providerinterval !== void 0) {
    ...
    return downloadSnapshot(args.providerurl, runtime, headers).then(
      (nodes) => main(prepared, args, nodes)
    );
  }
  globalThis.main = runMain;
})();
