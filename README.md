...
| `tun` | 启用 TUN 模式 | `false` |
| `threshold` | 某地区节点数量低于该值时不生成基础地区组；已生成基础组的地区不再重复列入 `手动选择`；不影响额外地区组 | `2` |
...
- 达到 `threshold` 并生成基础地区组后，该地区节点不再重复列入 `手动选择`；低于阈值、未识别地区的节点仍保留。没有剩余候选节点时不生成空的 `手动选择` 组。额外组本身不触发这项去重。
